...
# ----- Parse options ------
option_list = list(
  make_option(c("-d", "--plotsdir"),
              help = "path to directory to save plots"),
  make_option(c("-f", "--plotfuns"),
              help = "path to plotting functions",
              default = ""),
  make_option(c("-l", "--logfile"),
              help = "path to logfile"),
  make_option(c("-u", "--labkeyurlbase"),
              help = "labkey.url.base",
              default = "https://dataspace.cavd.org")
);
...
r <- try({
  ...
  nabmab <- labkey.selectRows(labkey.url.base,
                              folderPath = "/CAVD",
                              schemaName = "study",
                              queryName = "NABMAb",
                              colNameOpt = "fieldname",
                              method = "GET")
  virIds <- labkey.selectRows(labkey.url.base,
                              labkey.url.path,
                              "CDS",
                              "nabantigen",
  ...
  # mabMixNames <- labkey.selectRows(labkey.url.base,
  #                                  labkey.url.path,
  #                                  "CDS",
  #                                  "MAbMixMetadata",
  #                                  colNameOpt = "fieldname",
...
x <- lapply(c("rainbow", "gray"), function(paletteName) {
  ...
  # Mab vs all viruses
  res_m <- lapply(mabs, function(x) {
    c <- try({
      mab <- x
      dir <- file.path(plotsDir, paletteName, mab)
      if (!dir.exists(dir)) dir.create(dir, recursive = TRUE)
      createNeutralizationThumbnail(nabmab,
    ...
    if ("try-error" %in% class(c)) {
      dev.off()
      logger("WARNING", x, "vs all viruses failed:", c)
    }
    return(c)
  })
  ...
  res_v <- lapply(viruses, function(x){
    c <- try({
      virus <- x
      if (!dir.exists(dir)) dir.create(dir, recursive = TRUE)
      createNeutralizationThumbnail(nabmab,
    ...
    if ("try-error" %in% class(c)) {
      dev.off()
      logger("WARNING", x, "vs all mabs failed:", c)
    }
    return(c)
  })
