...
via `mabGridSummary`. See other documentations associated with this
...
Subset dataset to only the viruses that are analyzed for all the selected mAbs.
...
We will a set of viruses with the most mAb/virus combinations.
...
Calculate `magnitude` is the titer IC50 concentration. `breadth` is
...
### Which virsues do not have an IC50 at or below our lowest 3rd quartile magnitude?

```{r}

qVal <- minMagQuart[["75"]][[1]]

mbSel50 <- mb50[magnitude > qVal,]

virNeut <- mbSel50[,.(virus, mab_mix_label, magnitude)]
virNeut <- dcast(virNeut, virus~mab_mix_label, value.var = "magnitude")
virNeut <- virNeut[apply(virNeut[, -1, with = F], 1, function(x) all(!is.na(x))), virus]
virNeut

```

### What mAbs were tested against those viruses and showed lower IC50 results?
